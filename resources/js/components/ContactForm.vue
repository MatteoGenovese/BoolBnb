<template>
    <div class="my-5">
        <form class="d-flex flex-column w-50 m-auto" action="#" @submit.prevent="sendMessage()" method="POST">
            
            <label for="user">Inserisci il tuo nome completo</label>
            <input type="text" name="user" id="user" v-model="username">
            <label for="email">Inserisci la tua email *</label>
            <input type="email" name="email" id="user" v-model="userEmail">
            <label for="contact-message">Messaggio *</label>
            <textarea name="message" id="contact-message" cols="30" rows="10" v-model="contactMessage"></textarea>
